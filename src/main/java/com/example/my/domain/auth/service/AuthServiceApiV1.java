...
import com.example.my.model.user.entity.UserEntity.UserEntityBuilder;
...
public class AuthServiceApiV1 {
    ...
    public ResponseEntity<?> join(ReqJoinDTO dto) {
        ...
        //  리파지토리에서 아이디로 유저 찾기
        Optional<UserEntity> userEntityOtional = userRepository.findById(dto.getUser().getId());
        
        
        
        //  있으면 (이미 존재하는 아이디입니다.) 메시지 리턴
        if(userEntityOtional.isPresent()){
            return new ResponseEntity<>(
                ResponseDTO.builder()
                    .code(1)
        ...
        userRepository.save(userEntity); //이걸하기 위해 오지게 함
